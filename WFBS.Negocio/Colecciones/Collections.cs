...
namespace WFBS.Negocio
{
    ...
    {/* Usuario */
        ...
        private List<Negocio.Usuario> GenerarListado(List<DALC.USUARIO> usuariosBDD)
        {
            List<Negocio.Usuario> usuariosController = new List<Usuario>();

            foreach (DALC.USUARIO item in usuariosBDD)
            {
                Negocio.Usuario us = new Usuario();
        ...
        private List<Negocio.Area> GenerarListado(List<DALC.AREA> areasBDD)
        {
            List<Negocio.Area> areasController = new List<Area>();

            foreach (DALC.AREA item in areasBDD)
            {
                Negocio.Area ar = new Area();
        ...
        private List<Negocio.PerfilesdeCargo> GenerarListado(List<DALC.PERFIL_DE_CARGO> perfilesdecargoBDD)
        {
            List<Negocio.PerfilesdeCargo> perfilesdecargoController = new List<PerfilesdeCargo>();

            foreach (DALC.PERFIL_DE_CARGO item in perfilesdecargoBDD)
            {
                Negocio.PerfilesdeCargo pc = new PerfilesdeCargo();
        ...
        {
            List<DALC.PERFIL> perfilesBDD = CommonBC.ModeloWFBS.PERFIL.ToList();
            return GenerarListadoPerfil(perfilesBDD);
        }

        private List<Perfil> GenerarListadoPerfil(List<PERFIL> perfilesBDD)
        {
            List<Negocio.Perfil> perfilesController = new List<Perfil>();

            foreach (DALC.PERFIL item in perfilesBDD)
            {
                Negocio.Perfil p = new Perfil();
        ...
        private List<Negocio.Competencia> GenerarListadoCompetencia(List<DALC.COMPETENCIA> CompetenciaBDD)
        {
            List<Negocio.Competencia> competenciasController = new List<Competencia>();

            foreach (DALC.COMPETENCIA item in CompetenciaBDD)
            {
                Negocio.Competencia com = new Competencia();
        ...
        {
            List<DALC.HABILIDAD> habilidadesBDD = CommonBC.ModeloWFBS.HABILIDAD.ToList();
            return GenerarListado(habilidadesBDD);
        }

        private List<Negocio.Habilidad> GenerarListado(List<DALC.HABILIDAD> habilidadesBDD)
        {
            List<Negocio.Habilidad> habilidadesController = new List<Habilidad>();

            foreach (DALC.HABILIDAD item in habilidadesBDD)
            {
                Negocio.Habilidad hab = new Habilidad();
